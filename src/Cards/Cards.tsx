...
export const Cards = ({ isSelectionEnabled, handleTurnOver }: CardsProps) => {
    const { cards, upCards, setUpCards, pairedCards, clickNum, setClickNum } = React.useContext(CardContext);

    const handleClick = (index: number) => {
        const isClickable = isSelectionEnabled && !upCards.includes(index) && !pairedCards.includes(index);
        if (isClickable) {
        ...
        <div className="cards">
            {cards.map((card, index) =>
                <CardItem
                    ...
                    onClick={() => handleClick(index)} ></CardItem>)}
        </div>);
}